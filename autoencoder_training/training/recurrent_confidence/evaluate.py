...
CSV_OUTPUT_PATH = Path("evaluation_metrics_recurrent_conf_train_garden_eval_garden.csv")
...
def write_metrics_csv(metrics: dict, csv_path: Path):
    ...
    fieldnames = [
        ...
        "num_frames",
        "num_temporal_pairs",
        f"num_seq{SEQ_LEN}_windows",
        "mean_l1",
        "mean_gradient_l1",
        "mean_temporal_delta_l1",
        f"mean_sliding_seq{SEQ_LEN}_loss",
    ]
...
def run_evaluation(model, loader, device, checkpoint_epoch=None):
    ...
    for x, target, _conf in progress:
        ...
        frame_l1   = spatial_loss(pred, target)
        frame_grad = gradient_loss(pred, target)

        if prev_pred is None:
            frame_temp = torch.tensor(0.0, device=device)
        else:
            frame_temp = temporal_delta_loss(pred, target, prev_pred, prev_target)
        ...
        recent_preds.append(pred.detach())
        recent_targets.append(target.detach())
        ...
        if len(recent_preds) == SEQ_LEN:
            seq7_loss = sequence_loss(recent_preds, recent_targets, frame_weights)
    ...
    print(f"  Mean sliding seq-{SEQ_LEN}:        {mean_seq7:.6f}")
    print(f"  Sliding seq-{SEQ_LEN} windows:     {seq7_count}")
    ...
    return {
        ...
        f"mean_sliding_seq{SEQ_LEN}_loss": mean_seq7,
        "num_frames": frame_count,
        "num_temporal_pairs": temp_count,
        f"num_seq{SEQ_LEN}_windows": seq7_count,
    }
