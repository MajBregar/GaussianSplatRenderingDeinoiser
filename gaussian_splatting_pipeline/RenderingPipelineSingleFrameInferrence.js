...
export class RenderingPipelineSingleFrameInferrence {
    constructor({
        ...
        this.onnx_model.init().then(() => {
            ...
            this.runOptions = {
                preferredOutputLocation: { [this.outputName]: 'gpu-buffer' },
            };
    ...
    async inferrence_render() {
        ...
        try {
            ...
            console.error('ONNX inference failed:', err?.message ?? err);
            return false;
        } finally {
            this.inferenceInFlight = false;
        }
    ...
    async #renderInferenceFrameLocked() {
        ...
        const newOutput = results[this.outputName];

        if (!newOutput?.gpuBuffer) {
            newOutput?.dispose?.();
            throw new Error(`ONNX output '${this.outputName}' is not GPU-backed.`);
        }

        this.outputTensor?.dispose?.();
        this.outputTensor = newOutput;

        this.tensorToTextureConverter.render(
            this.outputTensor.gpuBuffer,
    ...
    #resizeInferenceBuffers() {
        const width  = this.canvas.width;
        const height = this.canvas.height;
        const inputBytes = 1 * 4 * width * height * 4;

        if (
            this.inferenceInputBuffer?.size === inputBytes &&
